...
public interface TaskExecutionContext {
     ...
     * previous resuming. Due to this the task developer should always check the {@link TaskExecutionContext#isStopped()} value after any <em>pauseIfRequested()</em> call. Note that a task execution can be paused only if the task {@link Task#canBePaused()} method returns <em>true</em>.
     */
     ...
     * gracefully its task execution, as soon as possible. Note that a task execution can be stopped only if the task {@link Task#canBePaused()} method returns <em>true</em>.
     * 
